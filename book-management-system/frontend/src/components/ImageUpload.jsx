...
.img-editor-modal {
  ...
  border-radius: 20px; width: 100%; max-width: 900px; max-height: 95vh;
...
@keyframes editorSlideUp { from{transform:translateY(24px);opacity:0} to{transform:translateY(0);opacity:1} }
.img-editor-header {
  display:flex; align-items:center; justify-content:space-between;
  padding:1rem 1.5rem; border-bottom:1px solid rgba(201,168,76,0.12); flex-shrink:0;
}
.img-editor-title {
...
.img-editor-title span {
  ...
  color:#c9a84c; background:rgba(201,168,76,0.1); padding:3px 10px; border-radius:99px;
  border:1px solid rgba(201,168,76,0.2); font-family:'DM Sans',system-ui,sans-serif;
}
.img-editor-body { display:grid; grid-template-columns:1fr 280px; flex:1; overflow:hidden; min-height:0; }
...
.crop-box {
  position:absolute; border:2px solid #c9a84c; box-sizing:border-box; cursor:move;
}
.crop-grid-line { position:absolute; background:rgba(201,168,76,0.3); pointer-events:none; }
.crop-handle {
  position:absolute; width:14px; height:14px; background:#c9a84c;
  border:2px solid #0a0908; border-radius:3px; z-index:10;
}
...
.img-editor-controls {
  border-left:1px solid rgba(201,168,76,0.1); display:flex; flex-direction:column;
  overflow-y:auto; background:#111009; pointer-events:all; position:relative; z-index:10;
}
.mode-tabs { display:flex; gap:4px; padding:0.75rem 1rem; border-bottom:1px solid rgba(201,168,76,0.1); flex-shrink:0; }
.mode-tab {
  flex:1; padding:7px 2px; background:transparent; border:1px solid rgba(201,168,76,0.12);
  border-radius:8px; color:#8a8070; font-size:0.62rem; font-weight:600;
  cursor:pointer; transition:all 0.18s; font-family:'DM Sans',system-ui,sans-serif;
...
.ctrl-title{font-size:0.6rem;font-weight:700;letter-spacing:0.18em;text-transform:uppercase;color:#c9a84c;margin-bottom:0.85rem;display:flex;align-items:center;gap:6px;font-family:'DM Sans',system-ui,sans-serif;}
...
.slider-value{font-size:0.68rem;color:#c9a84c;font-family:'DM Mono',monospace;background:rgba(201,168,76,0.08);padding:2px 7px;border-radius:4px;min-width:44px;text-align:center;}
.slider-reset-btn{background:none;border:none;color:#5a5448;cursor:pointer;font-size:0.75rem;padding:2px 5px;border-radius:3px;transition:color 0.15s;font-family:'DM Sans',system-ui,sans-serif;}
.slider-reset-btn:hover{color:#c9a84c;}
input[type=range].editor-slider{width:100%;height:4px;-webkit-appearance:none;appearance:none;background:#252118;border-radius:2px;outline:none;cursor:pointer;}
input[type=range].editor-slider::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;background:#c9a84c;border-radius:50%;cursor:pointer;box-shadow:0 2px 6px rgba(201,168,76,0.4);transition:transform 0.15s;}
...
.filter-btn{padding:8px 6px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:8px;color:#8a8070;font-size:0.68rem;font-weight:600;cursor:pointer;transition:all 0.15s;text-align:center;font-family:'DM Sans',system-ui,sans-serif;}
...
.aspect-btn{padding:7px 4px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:7px;color:#8a8070;font-size:0.68rem;font-weight:600;cursor:pointer;transition:all 0.15s;text-align:center;font-family:'DM Sans',system-ui,sans-serif;}
...
.crop-info-box{margin-top:0.9rem;padding:10px 12px;background:rgba(201,168,76,0.06);border-radius:8px;border:1px solid rgba(201,168,76,0.15);font-size:0.7rem;color:#8a8070;font-family:'DM Sans',system-ui,sans-serif;line-height:1.7;}
.crop-info-box strong{color:#c9a84c;}
.crop-reset-btn{width:100%;margin-top:0.75rem;padding:8px;background:rgba(201,168,76,0.06);border:1px solid rgba(201,168,76,0.2);border-radius:8px;color:#c9a84c;font-size:0.75rem;font-weight:600;cursor:pointer;transition:all 0.15s;font-family:'DM Sans',system-ui,sans-serif;}
...
.transform-btn{padding:10px 6px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:8px;color:#8a8070;font-size:1.1rem;cursor:pointer;transition:all 0.15s;display:flex;flex-direction:column;align-items:center;gap:4px;}
.transform-btn span{font-size:0.6rem;font-weight:600;letter-spacing:0.05em;text-transform:uppercase;font-family:'DM Sans',system-ui,sans-serif;pointer-events:none;}
.transform-btn:hover{color:#c9a84c;border-color:rgba(201,168,76,0.3);background:rgba(201,168,76,0.06);}
.img-editor-footer{display:flex;align-items:center;justify-content:space-between;padding:0.9rem 1.5rem;border-top:1px solid rgba(201,168,76,0.12);background:#0e0c09;flex-shrink:0;gap:1rem;}
.editor-info{font-size:0.7rem;color:#5a5448;font-family:'DM Mono',monospace;}
.editor-footer-btns{display:flex;gap:8px;}
.editor-btn-cancel{padding:9px 18px;background:transparent;border:1px solid rgba(255,255,255,0.1);border-radius:8px;color:#8a8070;font-size:0.82rem;font-weight:600;cursor:pointer;transition:all 0.15s;font-family:'DM Sans',system-ui,sans-serif;}
.editor-btn-cancel:hover{color:#ede8dc;border-color:rgba(255,255,255,0.25);}
.editor-btn-reset{padding:9px 18px;background:rgba(224,82,82,0.08);border:1px solid rgba(224,82,82,0.2);border-radius:8px;color:#e05252;font-size:0.82rem;font-weight:600;cursor:pointer;transition:all 0.15s;font-family:'DM Sans',system-ui,sans-serif;}
.editor-btn-reset:hover{background:rgba(224,82,82,0.15);}
.editor-btn-apply{padding:9px 26px;background:linear-gradient(135deg,#9a7a2e,#e8c96a);border:none;border-radius:8px;color:#0a0908;font-size:0.82rem;font-weight:700;cursor:pointer;transition:all 0.2s;font-family:'DM Sans',system-ui,sans-serif;box-shadow:0 4px 16px rgba(201,168,76,0.3);}
.editor-btn-apply:hover{transform:translateY(-1px);box-shadow:0 8px 24px rgba(201,168,76,0.45);}
.upload-zone{border:2px dashed rgba(201,168,76,0.2);border-radius:12px;padding:2.5rem 1.5rem;text-align:center;cursor:pointer;transition:all 0.25s;background:rgba(255,255,255,0.01);}
.upload-zone:hover,.upload-zone.drag-over{border-color:#c9a84c;background:rgba(201,168,76,0.04);}
.upload-icon-wrap{width:52px;height:52px;background:rgba(201,168,76,0.08);border:1px solid rgba(201,168,76,0.2);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.4rem;margin:0 auto 0.9rem;transition:all 0.25s;}
...
.upload-thumb-wrap img{max-height:180px;max-width:100%;border-radius:10px;border:1px solid rgba(201,168,76,0.2);display:block;box-shadow:0 8px 24px rgba(0,0,0,0.4);}
.thumb-actions{position:absolute;inset:0;background:rgba(0,0,0,0.6);border-radius:10px;display:flex;align-items:center;justify-content:center;gap:8px;opacity:0;transition:opacity 0.2s;}
.upload-thumb-wrap:hover .thumb-actions{opacity:1;}
.thumb-action-btn{padding:7px 14px;border:none;border-radius:7px;font-size:0.75rem;font-weight:700;cursor:pointer;font-family:'DM Sans',system-ui,sans-serif;transition:all 0.15s;}
...
.close-x-btn{background:none;border:none;color:#8a8070;font-size:1.5rem;cursor:pointer;padding:4px 10px;border-radius:6px;transition:all 0.15s;line-height:1;}
.close-x-btn:hover{color:#ede8dc;background:rgba(255,255,255,0.06);}
@media(max-width:640px){
  .img-editor-body{grid-template-columns:1fr;}
  .img-editor-controls{border-left:none;border-top:1px solid rgba(201,168,76,0.1);max-height:220px;}
}
`;
...
const DEFAULT_ADJ = { brightness:100, contrast:100, saturation:100, hue:0, blur:0, zoom:1, rotate:0, flipH:false, flipV:false };
...
function ImageEditor({ src, onApply, onCancel }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      drawCanvas(img, { ...DEFAULT_ADJ }, 'none');
    };
    img.src = src;
  }, [src]);
  ...
  const drawCanvas = (img, a, f) => {
    ...
    const maxW = (parent?.clientWidth  || 560) - 20;
    const maxH = (parent?.clientHeight || 460) - 20;
    ...
    ctx.filter = [`brightness(${a.brightness}%)`,`contrast(${a.contrast}%)`,`saturate(${a.saturation}%)`,`blur(${a.blur*0.4}px)`,`hue-rotate(${a.hue}deg)`,fp.css].filter(Boolean).join(' ');
  ...
  const onMouseMove = useCallback((e) => {
    ...
    if (type === 'move') {
      ...
      if (ar) {
        ...
        h=clamp(h,MIN_CROP,ch-y); w=clamp(w,MIN_CROP,cw-x);
      }
    }
    setCrop({ x, y, w, h });
  }, [aspect]);
  ...
  const handleApply = () => {
    ...
    ctx.filter = [`brightness(${adj.brightness}%)`,`contrast(${adj.contrast}%)`,`saturate(${adj.saturation}%)`,`blur(${adj.blur*0.4}px)`,`hue-rotate(${adj.hue}deg)`,fp.css].filter(Boolean).join(' ');
  ...
  const handleReset = () => {
    ...
    setFilter('none');
    setAspect('free');
    if (canvasRef.current) setCrop({ x:0, y:0, w:canvasRef.current.width, h:canvasRef.current.height });
  };
    ...
    <div className="img-editor-overlay">
      <div className="img-editor-modal" onClick={stopProp} onMouseDown={stopProp}>
        ...
        <div className="img-editor-body">
          <div className="img-editor-canvas-area"
          ...
          >
            <div className="crop-wrapper" ref={wrapperRef}>
              ...
              {tab === 'crop' && crop.w > 0 && (
                <>
                  <div style={{position:'absolute',top:0,left:0,width:cw,height:crop.y,background:'rgba(0,0,0,0.55)',pointerEvents:'none'}} />
                  <div style={{position:'absolute',top:crop.y+crop.h,left:0,width:cw,height:ch-(crop.y+crop.h),background:'rgba(0,0,0,0.55)',pointerEvents:'none'}} />
                  <div style={{position:'absolute',top:crop.y,left:0,width:crop.x,height:crop.h,background:'rgba(0,0,0,0.55)',pointerEvents:'none'}} />
                  <div style={{position:'absolute',top:crop.y,left:crop.x+crop.w,width:cw-(crop.x+crop.w),height:crop.h,background:'rgba(0,0,0,0.55)',pointerEvents:'none'}} />
            ...
            {tab==='crop' && (
              <div className="ctrl-section">
                ...
                <div className="crop-info-box">
                  ...
                  • Select ratio above to lock shape<br/>
...
function ImageUpload({ value, onChange, existingUrl }) {
  ...
  const handleApply = (editedFile, editedUrl) => { setPreview(editedUrl); onChange(editedFile); setEditor(false); };
  ...
  return (
    <div>
      {showEditor && rawSrc && <ImageEditor src={rawSrc} onApply={handleApply} onCancel={()=>setEditor(false)} />}
        ...
        <div style={{marginBottom:10}}>
          <div className="upload-thumb-wrap">
            ...
            <div className="thumb-actions">
              <button type="button" className="thumb-action-btn edit" onClick={()=>{setRawSrc(preview);setEditor(true);}}>🎨 Edit</button>
      ...
      {showExisting && (
        <div style={{marginBottom:10}}>
          <div className="upload-thumb-wrap">
            ...
            <div className="thumb-actions">
              <button type="button" className="thumb-action-btn edit" onClick={()=>{setRawSrc(existingUrl);setEditor(true);}}>🎨 Edit</button>
              <button type="button" className="thumb-action-btn remove" onClick={handleRemove}>✕ Remove</button>
            </div>
          </div>
          <p style={{marginTop:8,fontSize:'0.75rem',color:'#8a8070',fontFamily:'DM Sans,system-ui,sans-serif'}}>Hover over image to edit or remove</p>
        </div>
      )}
